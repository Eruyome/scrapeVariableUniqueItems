...
urls.forEach(function(url, index){
	...
	request(options, function (error, response, body) {
		...
		var prefix = $('#Prefix').parentsUntil('.bodyContent');
		var suffix = $('#Suffix').parentsUntil('.bodyContent');
		var affixTypes = [prefix, suffix];
		var data = { 'prefix':[], 'suffix':[]}

		affixTypes.forEach(function(affixType, k){
			var modsRaw = $(affixType).find('th > em').filter('.-mod');
			...
			modsRaw.each(function(i, mod){
				...
				if (pos < 0) {
					...
					if (pos2 > 0) {
						arr = mod.split(",");
						// split mods only when every part has its own value
